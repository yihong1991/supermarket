$(function(){
	...
	$("input.plus").live("click",function(){//减少数量
		var t=$(this)
		var id=t.attr("data-role");
		var obj=t.next("#thisnum");
		var curReserves=Number(obj.text());	//购物车内商品数量
	
		$.ajax({ 
			type:"post",
			url: "ajax/docart.php?act=plus", 
			beforeSend:function(){
				Numloading(t,0);
			},
			data:{id:id}, 
	...
	$("input.upcart").live('click',function(){//增加数量
		...
		$.ajax({
			type : "POST",
			url : "ajax/docart.php?act=upcart",
			beforeSend:function(){
				Numloading(ad,0);
			},
			data : {id:id},
